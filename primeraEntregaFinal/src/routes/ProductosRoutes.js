...
productsRouter.put('/:id', async function(req, res) {
        ...
        if (productoBody && productoBody.nombre && productoBody.precio && productoBody.codigo) {
            let productoUpdate = await productosContainer.update(req.params.id ,productoBody.nombre, productoBody.descripcion, productoBody.codigo, productoBody.precio, productoBody.stock, productoBody.fecha, productoBody.img);
